...
Namespaces use subdomain notation with a version component:
- `intel.emphere.dev/v1` - Emphere intelligence extensions
- `custom.vendor.example/v1` - Vendor-specific extensions
...
Tools SHOULD NOT require extensions to function. If `extensions["intel.emphere.dev/v1"]` is missing, use default prioritization (e.g., CVSS score alone) instead of failing.
...
|-----------|-------------|---------------|
| `intel.emphere.dev/v1` | CVE intelligence from Emphere | This document |

---

## `intel.emphere.dev/v1` Extension

The `intel.emphere.dev/v1` extension provides CVE intelligence analysis from Emphere. It answers four critical questions:
...
See [examples/kb/cve-mapping-with-intel.yaml](../examples/kb/cve-mapping-with-intel.yaml) for a complete example of a CveMapping with the `intel.emphere.dev/v1` extension.

---

## Versioning

Extension schemas are versioned using the subdomain pattern with version suffix. Breaking changes to an extension schema SHOULD result in a new version (e.g., `intel.emphere.dev/v2`).

Current versions:
- `intel.emphere.dev/v1` - current (this document)

---

## Intelligence Quality & Feedback

The `intel.emphere.dev/v1` extension is generated by Emphere and continuously improved through multiple feedback channels:
